...
class data
{
	...
	vector<int> need;
	//vector<int> avail;
	//static vector<int>total_allocated;//total no of process allocated
	data()
	{
   	...
   	void get_alloc()
   	{
   		cout<<"allocation  ";
	  		...
	  		for(int i=0;i<n;i++)
	  		{
      				 ...
      				 cin>>a1;
					alloc.push_back(a1);
					//total_allocated[i]+=alloc[i];//total no of allocated process
	  		}
 ...
 /*  	static void total_alloc_cal()
{
	for(int i=0;i<n;i++)
			total_allocated.push_back(0);
}
static void total_alloc_cal_f()
{
	for(int i=0;i<n;i++)
			total_allocated[i]+=alloc[i];
}
...
int main()
{
	...
	//data::total_alloc_cal();//static functio
   ...
   cout<<endl;
	...
	int count=0;
	//int flag2=1;//if safe sequence or not
	int count2=0;
	vector<int> safe;
	for(int j=0;j<=m;j++)
	{  
		...
		//process2.erase(process2.begin()+j);
		//for(int i=0;i<n;i++)
			//process2[j].need[i]=9999;
		...
		//cout<<"count ="<<count<<"count2= "<<count2<<" j= "<<j<<endl;
