...
*avg_tat=0;
 *avg_wt=0;
while(jump<=t_time)//RUN AS MANY TIME AS THAT OF BURST TIME/TIME QUANTUM
{
   ...
   cout<<"jump = "<<jump<<endl;
    ...
    if(flag)//IF TILL TIME JUMP ANY PROCESS FOUND
    {
      ...
      if(v2[u1].bt>0)//IF PROCESS IS STILL REMAINING
        v2[u1].at=jump+1;//push at end of queue 
         //v2[u1].at=tq*n+count;//this process start when all other process completes at least once
      else// SOMETIME NO PROCESS FOUND AS JUMP CANNOT INCREASE MORE THAN TIME QUANTOM
      {
         v2[u1].at=20000;//random large value 
         v1[u1].ct=jump;//PROCESS HAS FINISHED
         cout<<"Process P"<<u1+1<<"  complete at "<<time<<endl;
      }
       cout<<"Process P"<<u1+1<<" burst time "<<v2[u1].bt<<" jump = "<<jump<<endl;
